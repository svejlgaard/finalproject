...
def summary_analyze(data: pd.DataFrame):
	...
	g = sb.PairGrid(data, palette=list("red"), y_vars=["RV"], x_vars=list(data.columns.values.tolist()))
	...
	g.map(corrfunc)
	plt.savefig(f'Pearson_{date}.pdf')

date = 'etalon_jitter_26Mar_whitebox_etalon.ccfSum-telemetry'

dataframe = pd.read_csv(f'data/{date}.csv')

summary_analyze(dataframe)